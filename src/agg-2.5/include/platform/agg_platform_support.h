...
#if __ANDROID__
#define ERROR_PRINT(...) do{ __android_log_print(ANDROID_LOG_INFO, __FILE__, __VA_ARGS__ ); } while (false)
#else
...
namespace agg
{
    ...
    public:
        ...
        //--------------------------------------------------------------------
        // Sound support... why not?
        bool load_music(unsigned idx, const char* file);
        bool load_sound(unsigned idx, const char* file);
        void play_music(unsigned idx, unsigned vol);
        void play_sound(unsigned idx, unsigned vol);
        ...
        virtual void enter_foreground() {
           on_ctrl_change();

        };
        virtual void enter_background() {
           wait_mode(true);
        }
